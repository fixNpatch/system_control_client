...
func GetProcessNameMap() map[uint32]string {
	...
	if err != nil {
		fmt.Printf("Fail to syscall CreateToolhelp32Snapshot: %v", err)
		return nil
	}
	...
	if err = syscall.Process32First(snapshot, &procEntry); err != nil {
		fmt.Printf("Fail to syscall Process32First: %v", err)
		return nil
	}
	...
	for {
		...
		if err = syscall.Process32Next(snapshot, &procEntry); err != nil {
			...
			fmt.Printf("Fail to syscall Process32Next: %v", err)
			return nil
		}
	}
}
...
func metrics(funcName string) func(now time.Time) {
	return func(now time.Time) {
		fmt.Printf("Processing [%v] costs %v", funcName, time.Since(now))
	}
}
